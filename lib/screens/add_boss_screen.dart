  ...
  Future<void> _save() async {
    ...
    if (name.isEmpty || country.isEmpty || opening <= 0) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            "Please fill required fields (Name, Country, Opening Balance).",
          ),
        ),
      );
      return;
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              TextField(
                controller: _name,
                decoration: _dec("Boss Name (required)", Icons.person),
              ),
              const SizedBox(height: 12),
              TextField(
                controller: _country,
                decoration: _dec(
                  "Country (required) e.g. Malaysia",
                  Icons.public,
                ),
              ),
              const SizedBox(height: 12),
              ...
              TextField(
                ...
                decoration: _dec("Phone (optional)", Icons.call),
              ),
              const SizedBox(height: 12),
              TextField(
                controller: _address,
                decoration: _dec("Address (optional)", Icons.home),
              ),
