...
class _BossDetailScreenState extends State<BossDetailScreen> {
  ...
  Future<void> _edit(Boss b) async {
    await Navigator.of(context).push(
      MaterialPageRoute(builder: (_) => AddBossScreen(editBoss: b)),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (b == null) {
      return Scaffold(
        appBar: AppBar(
          title: const Text("Boss Detail", style: TextStyle(fontWeight: FontWeight.w900)),
          centerTitle: true,
      ...
      body: Padding(
        ...
        child: Column(
          children: [
            ...
            const SizedBox(height: 10),
            _InfoTile(label: "Address", value: b.address.isEmpty ? "-" : b.address),
            const SizedBox(height: 10),
              ...
              children: [
                  ...
                  child: _ActionBtn(
                    ...
                    onTap: () {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(content: Text("Next: Transactions screen (locked logic later)")),
                      );
                    },
                  ),
                ),
                ...
                Expanded(
                  child: _ActionBtn(
                    ...
                    onTap: () {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(content: Text("Next: Reports screen (locked logic later)")),
                      );
                    },
                  ),
                ),
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(
        children: [
          Expanded(
            child: Text(label, style: const TextStyle(fontWeight: FontWeight.w900, color: Colors.black54)),
          ),
...
class _ActionBtn extends StatelessWidget {
  ...
  const _ActionBtn({required this.icon, required this.text, required this.onTap});
